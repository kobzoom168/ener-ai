...
async def assemble_board_bg(motion: str) -> None:
    ...
    try:
        ...
        if motion in ("talk", "talk1"):  # ── ละครพูด: dialogue → OmniHuman lip-sync ──
            ...
            mp4 = await assemble_talking(talk_shots, b.get("characters", []), out, aspect,
                                         log=_alog, genre=b.get("genre", ""))
            if mp4:
                STORY_STATE["mp4"] = mp4
                _save_board()
...
_STORY_SYSTEM = """คุณคือ "นักเขียนบทหนังสั้นมืออาชีพ" (คิดเหมือนคนเขียนบทหนัง/ซีรีส์จริง) เชี่ยวชาญเรื่องเล่าไทยทุกแนว
งาน: ออกแบบบทที่ "ดีที่สุด" ให้พอดีกับ แนวเรื่อง/ความยาว(จำนวนช็อต)/จำนวนตัวละคร ที่ผู้ใช้กำหนด แล้วแตกเป็นสตอรี่บอร์ดทีละช็อต

เขียนบทระดับมืออาชีพ (คิดโครงก่อนเขียน):
- คิดก่อนว่า "เรื่องแบบไหนดีสุด" ภายใต้เงื่อนไข แล้ววางโครงให้พอดีจำนวนช็อต: เปิด(hook สะดุดใน 1-2 ช็อตแรก) → ก่อปม/สร้างความตึง → จุดพีค → หักมุม/จบกระแทกใจ-ติดตา
- ตัวละครมีมิติ: บุคลิก/ความต้องการ/ปมชัด แต่ละคนพูดคนละสไตล์ (เสียงตัวละครต่างกัน ไม่พูดเหมือนกันหมด)
- บทพูด (dialogue) ต้องเป็นธรรมชาติแบบคนไทยพูดจริง มีอารมณ์/subtext กระชับ ไม่ท่องอธิบาย (ไม่ exposition แข็งๆ) — พูดให้เหมือนคนจริงในสถานการณ์นั้น
- คุมโทน/จังหวะตามแนวเรื่อง (สยองขวัญ=ค่อยๆ ก่อความกลัว มีจังหวะเงียบก่อนผวา / ดราม่า=อารมณ์ลึกจริงใจ / ตลก=จังหวะมุกเป๊ะ)
- ทุกช็อตต้องขยับเรื่องไปข้างหน้า (ห้ามย่ำอยู่กับที่/พูดน้ำท่วมทุ่ง)
- **เรื่องต้องจบสมบูรณ์ + เข้าใจง่าย**: มีจุดเริ่ม-กลาง-จบครบ คลี่คลายปม/เฉลยชัด คนดูดูจบต้องเข้าใจว่าเกิดอะไร ไม่ค้างคา ไม่งง · concept ต้องเป็นรูปธรรมเข้าใจง่าย (ผี/เหตุการณ์ต้องชัดว่าคืออะไร) **ห้ามนามธรรม/ศัพท์เทคนิคซับซ้อนจนคนดูงง**
- **ภาษา (สำคัญมาก):** dialogue และ narration ต้องเป็น **ภาษาไทยล้วน 100%** — ห้ามมีอักษรจีน/ญี่ปุ่น/ต่างชาติปนเด็ดขาด · **เลี่ยงศัพท์เทคนิคภาษาอังกฤษในบทพูด** (เช่น log, ping, IP, server, error) เพราะเสียงพากย์อ่านอังกฤษแล้วเพี้ยนฟังไม่เหมือนคนไทย — ให้เขียนเป็นคำไทย/คำอ่านไทย ("ล็อก" "ปิง" "ไอพี" "เซิร์ฟเวอร์") หรือเลี่ยงไปเลย พูดแบบคนทั่วไปเข้าใจ
- **จังหวะ+อารมณ์การพูด:** เขียน dialogue ให้มีจังหวะ (ใส่ "..." เว้นวรรคหยุดคิด/ลังเล, เน้นคำสำคัญ) และสื่ออารมณ์ในบท (ตกใจ/กลัว/สั่นเสียง) ให้เสียงพากย์อ่านได้มีอารมณ์
- **มุมกล้องหลากหลาย น่าสนใจ:** อย่าให้ทุกช็อตเป็น close-up หน้าตรงเหมือนกันหมด — ใส่มุม/ระยะหลากหลายให้ VDO น่าดู (wide establishing, medium, close-up, insert ช็อตมือ/จอ/วัตถุสำคัญ, มุมต่ำ low-angle, มุมสูง high-angle, **dutch angle เอียงตอนตึงเครียด/สยอง**) สลับกันไปตามจังหวะเรื่อง

**ภาพต้องเข้ากับบทเป๊ะ (สำคัญมาก):** image_prompt ของแต่ละช็อตต้อง "ตรงกับสิ่งที่พูด/เล่าในช็อตนั้นเป๊ะ" — สีหน้า/ท่าทาง/แอ็กชัน/บรรยากาศในภาพ ต้องสื่ออารมณ์และเหตุการณ์ของบทช็อตนั้น **สีหน้าต้องแสดงอารมณ์แรงและชัดเจนมาก (very expressive, strong emotion — เช่น กลัวสุดขีด/ตกใจสุด/เจ็บปวด/สะเทือนใจ ห้ามหน้านิ่งเฉย)** ให้คนดูภาพแล้วเข้าใจ+อินกับอารมณ์

หลักการภาพ:
- ตัวละครต้องคงที่ทั้งเรื่อง: บรรยายหน้าตา/ชุด/ลักษณะเด่นแต่ละตัวให้ชัด (เพื่อใช้สร้างชีตตัวละครอ้างอิง)
- ภาพ "สมจริงสุด + บริบทไทยแท้": คนไทย ผิว/ผม/ชุดไทย, สถานที่/สถาปัตยกรรมไทยตามเรื่อง
- image_prompt ต้องตรงกับฉาก/ยุคสมัยจริง **อย่าใส่วัตถุที่ไม่เข้ากับสถานที่/บริบท** (เช่น ห้ามใส่พระพุทธรูป/ศาลพระภูมิ/ของโบราณ ในห้อง server/ออฟฟิศ/ที่ทันสมัย ถ้าเรื่องไม่ได้ระบุ)
- พรอมต์ภาพเป็นภาษาอังกฤษ cinematic photorealistic ระบุ: ใคร/สีหน้าอารมณ์/ทำอะไร/ที่ไหน/มุมกล้อง/แสง
- ห้ามให้ image_prompt มีป้าย/ตัวหนังสือที่อ่านได้ (โมเดลภาพเขียนอักษรไทยไม่ได้ จะเพี้ยน) — เลี่ยงฉากที่ต้องมีป้าย/ตัวอักษร หรือระบุว่าป้ายเปล่า (blank unmarked sign)
...
async def generate_story(topic: str, n_shots: int = 8, characters: int = 2,
    ...
    prompt = (
        ...
        + (f"แนวเรื่อง: {tone}\n" if tone else "")
        + (f"สถานที่/ฉากหลัก: {loc} — **ทุก image_prompt ต้องบรรยายสถานที่/ห้องเดิมเป๊ะ (เฟอร์นิเจอร์ สี แสง โทน มุมเดียวกัน) "
           f"เหมือนถ่ายในที่เดียวกันทั้งเรื่อง ห้ามเปลี่ยนห้อง/ฉากไปมา** — ใส่รายละเอียดสถานที่ซ้ำในทุกช็อต\n" if loc else "")
        + ("โหมดละครพูด (แบบ shot-reverse-shot เหมือนละครจริง):\n"
           "- แบ่งบทสนทนาเป็น 'ช็อตสลับหน้า' — **1 ช็อต = ตัวละครพูด 1 คนเท่านั้น** (dialogue มี speaker เดียว 1-2 ประโยคสั้นๆ)\n"
           "- **image_prompt ของช็อตพูด = close-up ใบหน้าตัวละครคนนั้น 'คนเดียวเต็มเฟรม' (solo portrait)** — "
           "ในเฟรมมีแค่คนนี้คนเดียวเท่านั้น **ห้ามมีคนอื่น/ไหล่คนอื่น/หลังหัวคนอื่น/เงาคนอื่นในเฟรมเด็ดขาด** "
           "(ห้าม over-the-shoulder ห้าม two-shot) หันหน้าเฉียง 3/4 เห็นหน้าชัด · "
           "**เปลี่ยนมุม/ระยะ close-up ให้ต่างกันแต่ละช็อต ไม่ซ้ำจำเจ** (เช่น close-up มุมต่ำ, dutch angle เอียงตอนตกใจ, extreme close-up ดวงตา, ระยะใกล้-กลางสลับกัน) · "
           "**ใส่สีหน้าที่มีอารมณ์ชัดตามบทพูด** (เช่น กังวล/ตกใจ/เศร้า/โกรธ — emotional facial expression) เพื่อให้ดูมีอารมณ์\n"
           "- ช่อง \"characters\" ของช็อตพูด = ใส่ชื่อ 'คนพูดคนเดียว'\n"
           "- สลับ close-up ไปมาระหว่างตัวละครตามบทสนทนา (A พูด→close-up A, B ตอบ→close-up B)\n"
           "- dialogue ต่อเนื่องเป็นเรื่องเดียวกัน สื่อความหมาย · ใส่ช็อต establishing/บรรยากาศ (narration) คั่นได้บ้างให้เข้าใจฉาก\n" if dialogue else "")
...
async def narrate_shots(shots: list[dict]) -> tuple[list[str], list[float]]:
    ...
    def _one(i: int, shot: dict) -> tuple[str, float]:
        ...
        try:
            _synth_voice(_shot_text(shot), out, _VOICE_NARRATION)
...
import os as _os
# ElevenLabs voices (กบ supplied). Configurable via env if ever changed.
_VOICE_FEMALE = _os.environ.get("STORY_VOICE_FEMALE", "") or "Z3R5wn05IrDiVCyEkUrK"
_VOICE_MALE = _os.environ.get("STORY_VOICE_MALE", "") or "BzWc3iJ0MiRdqIo6RCvM"
_VOICE_NARRATION = _os.environ.get("STORY_VOICE_NARRATION", "") or "zxYcg6tHQ7MGettn8eau"


_AMBIENT = {
    "horror": "eerie low ambient drone, tense unsettling horror atmosphere, distant hum, faint wind, no music",
    "drama": "soft warm ambient room tone, gentle emotional atmosphere, very subtle, no music",
    "comedy": "light neutral room ambience, subtle and airy, no music",
}
...
async def gen_ambient(genre: str, out: str, seconds: int = 20) -> str | None:
    ...
    if not key:
        return None
    prompt = _AMBIENT.get(str(genre).strip().lower(),
                          "subtle cinematic ambient room tone atmosphere, no music")
    try:
...
async def assemble_talking(shots: list[dict], characters: list[dict], out_mp4: str,
                           aspect: str = "9:16", fps: int = 30, log=None, genre: str = "") -> str:
    ...
    for i, s in enumerate(shots):
        ...
        if not done:  # ── narration / fallback → Ken Burns + voice ──
            ...
            if narr:
                try:
                    _synth_voice(narr[:400], aud, _VOICE_NARRATION)
