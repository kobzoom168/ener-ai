...
async def generate_story(topic: str, n_shots: int = 8, characters: int = 2,
    ...
    prompt = (
        ...
        + ("โหมดละครพูด: **ตัวละครต้องพูดจริง (dialogue) ในช็อตส่วนใหญ่** — เขียนให้ตัวละครโต้ตอบกัน "
           "หรือพูดคนเดียว/รำพึงออกมาดังๆ · **เรื่องควรมีตัวละครโต้ตอบกัน (ไม่ใช่คนเดียวเงียบๆ)** · "
           "dialogue ต้องต่อเนื่องเป็นเรื่องเดียวกัน สื่อความหมาย (ไม่ใช่ประโยคลอยๆ) ระบุ speaker ตรงชื่อตัวละคร "
           "+ image_prompt ช็อตที่พูดเป็น close-up/medium เห็นหน้าชัด · narration ใส่เชื่อมเท่าที่จำเป็นให้เรื่องรู้เรื่อง\n" if dialogue else "")
